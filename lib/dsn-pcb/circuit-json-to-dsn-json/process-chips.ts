import type { AnyCircuitElement, PcbComponent, PcbPort, SourceComponentBase, SourcePort } from "circuit-json"
import type { DsnPcb } from "../types"

export function processChips(circuitElements: AnyCircuitElement[], pcb: DsnPcb) {
    // Group pcb_port by component
    const portsByComponent = new Map<string, PcbPort[]>()

    // Find all source components that are simple_chips
    const chipComponents = circuitElements.filter((e) => 
        e.type === "source_component" && e.ftype === "simple_chip"
    ...
    chipComponents.forEach(chip => {
        const componentPorts = circuitElements.filter((e): e is PcbPort => {
            ...
            const sourcePort = circuitElements.find(
                (sp): sp is SourcePort => sp.type === "source_port" && sp.source_port_id === e.source_port_id
    ...
    chipComponents.forEach(chip => {
        ...
        const existingImage = pcb.library.images.find(img => img.name === imageName)
        ...
        if (!existingImage) {
            ...
            if (!pcb.library.padstacks.find(p => p.name === `default_pad_${imageName}`)) {
                    ...
                    shapes: [
                        {
                            ...
                            layer: "F.Cu",
                            width: 0,
                            coordinates: [-300, 300, 300, 300, 300, -300, -300, -300, -300, 300]
                        }
                    ],
                    attach: "off"
                })
            ...
            const pcbComponent = circuitElements.find(
                (e): e is PcbComponent => e.type === "pcb_component" && e.source_component_id === chip.source_component_id
