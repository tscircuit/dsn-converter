...
import { parsePinDefinition } from "lib/utils/get-pin-number"
...
function processPin(nodes: ASTNode[]): Pin | null {
  try {
    ...
    const parsed = parsePinDefinition(nodes)
    if (!parsed) return null

    return {
      padstack_name: String(nodes[1].value),
      pin_number: parsed.pinNumber,
      x: parsed.x,
      y: parsed.y,
