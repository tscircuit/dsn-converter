...
import { type Matrix, applyToPoint, rotate } from "transformation-matrix"
    ...
    component.places.forEach((place) => {
      ...
      if (image.pins) {
        for (const pin of image.pins) {
          ...
          // Apply rotation to pin offset around component center
          const rotationRad = ((place.rotation || 0) * Math.PI) / 180
          const rotationTransform = rotate(rotationRad)
          const rotatedPinOffset = applyToPoint(rotationTransform, {
            x: pin.x,
            y: pin.y,
          })
