...
test("check that on hover all the trace and connected pads are correctly linked", async () => {
  ...
  expect(
    pcb_trace?.source_trace_id && source_trace?.source_trace_id,
  ).toBeTruthy()
  ...
  const expected_source_port_id = (
    pcb_smtpad as { pcb_port_id: string }
  ).pcb_port_id.replace("pcb_port_", "source_port_")
