...
test("preserves class circuit use_layer descriptors", () => {
  ...
  expect(dsnJson.network.classes[0].circuit.use_layers).toEqual([
    "F.Cu",
    "B.Cu",
  ])

  const stringified = stringifyDsnJson(dsnJson)
  expect(stringified).toContain("(use_layer F.Cu B.Cu)")

  const reparsed = parseDsnToDsnJson(stringified) as DsnPcb
  expect(reparsed.network.classes[0].circuit.use_layers).toEqual([
    "F.Cu",
    "B.Cu",
  ])
})
