...
test("preserves library directory descriptors", () => {
  ...
  expect(dsnJson.library.directory).toBe("./parts")

  const stringified = stringifyDsnJson(dsnJson)
  expect(stringified).toContain('(directory "./parts")')

  const reparsed = parseDsnToDsnJson(stringified) as DsnPcb
  expect(reparsed.library.directory).toBe("./parts")
})
